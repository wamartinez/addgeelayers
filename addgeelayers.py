        ...
        if result:
            ...
            if not name_layers:
                ...
                if valStart.daysTo(valEnd) <= 0:
                    ...
                    state_pca = self.dlg.CheckBPCA.checkState()
                    state_fc = self.dlg.CheckFCA.checkState()
                    state_tc = self.dlg.CheckBTC.checkState()
                    if np.array([state_ndvi,state_pca,state_fc,state_tc]).sum() == 0:
                        QMessageBox.information(self.dlg,"Message","Please choose an option to display")
                    else:
                        #get coordinates
                        ...
                        map_gee_layers(self,list_extent,valStart,valEnd,computation,state_ndvi,state_pca,state_fc,state_tc)
                        print("Done")
